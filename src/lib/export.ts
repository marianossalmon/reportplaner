...
/**
 * Load any image (URL, data URI, blob URL) through a browser canvas → always
 * produces a valid PNG dataURL that jsPDF can parse without "wrong PNG signature".
 */
...
function drawHeader(
  pdf: jsPDF,
  W: number,
  gebesaImg: LoadedImage | null,
  clientImg: LoadedImage | null,
  projectName: string,
  advisorName: string,
  versionLabel: string
) {
  ...
  // Gebesa logo — left, contained in 36×10 mm
  if (gebesaImg) {
  ...
  // Client logo — right, contained in 36×10 mm  ← más pequeño
  if (clientImg) {
    drawLogoContained(pdf, clientImg, W - 43, 6, 36, 10, true);
  }

  // Project + advisor centered
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(9);
...
function drawVersionPage(
  ...
  const TOP    = 26;
  const SIDE_W = 54;
  const DIV_X  = SIDE_W + 3;
  const CVS_X  = DIV_X + 4;
  const CVS_W  = W - CVS_X - 5;
  const CVS_H  = H - TOP - 12;
  ...
  const mBar = (label: string, pct: number) => {
    ...
    sy += 2.5;
    pdf.setFillColor(229, 226, 221);
    pdf.rect(7, sy, SIDE_W - 10, 2, 'F');
    const c = pct >= 70 ? [107, 142, 35] : pct >= 40 ? [255, 165, 0] : [220, 80, 60];
    pdf.setFillColor(c[0], c[1], c[2]);
    pdf.rect(7, sy, (SIDE_W - 10) * pct / 100, 2, 'F');
    sy += 7;
  };
  ...
  pdf.rect(CVS_X, TOP, CVS_W, CVS_H, 'F');
  pdf.setDrawColor(229, 226, 221);
  pdf.setLineWidth(0.3);
  ...
  if (vc.imgData) {
    ...
    const nH = img.naturalHeight || 650;
    const ratio = nW / nH;
    let dW = CVS_W - 4;
    let dH = dW / ratio;
    if (dH > CVS_H - 4) { dH = CVS_H - 4; dW = dH * ratio; }
    const ix = CVS_X + (CVS_W - dW) / 2;
    const iy = TOP   + (CVS_H - dH) / 2;
    pdf.addImage(vc.imgData, 'PNG', ix, iy, dW, dH);
  }

  pdf.setFont('helvetica', 'normal');
  pdf.setFontSize(5.5);
  pdf.setTextColor(160, 158, 145);
...
function drawComparisonPage(
  ...
  pdf.setFillColor(42, 42, 30);
  pdf.rect(8, TOP, W - 16, 7, 'F');
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(7);
  pdf.setTextColor(255, 255, 255);
  pdf.text(dict.summary.metric, 9 + colW * 0.05, TOP + 4.5);
  vcs.forEach((vc, i) => {
    pdf.text(vc.name, 8 + colW * (i + 1.5), TOP + 4.5, { align: 'center' });
  });
  ...
  rows.forEach((row, ri) => {
    ...
    if (ri % 2 === 1) {
      pdf.setFillColor(248, 247, 244);
      pdf.rect(8, ry, W - 16, 7, 'F');
    }
    pdf.setDrawColor(229, 226, 221);
    pdf.setLineWidth(0.1);
    pdf.line(8, ry + 7, W - 8, ry + 7);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(6.5);
    pdf.setTextColor(90, 90, 64);
    pdf.text(row.label, 9 + colW * 0.05, ry + 4.5);
    vcs.forEach((vc, vi) => {
      const val = (vc.metrics as any)[row.key];
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(6.5);
      pdf.setTextColor(42, 42, 30);
      pdf.text(String(val ?? '—'), 8 + colW * (vi + 1.5), ry + 4.5, { align: 'center' });
    });
  });
  ...
  vcs.forEach((vc, vi) => {
    ...
    const pts = axes.map((ax, i) => {
      ...
      const norm = ax.invert
        ? Math.max(0, 1 - raw / ax.max)
        : Math.min(1, raw / ax.max);
  ...
  vcs.forEach((vc, vi) => {
    const m = vc.metrics;
    const work = m.openSpace;
    const shared = m.confRooms + m.lounge;
    const amenities = m.dining + m.reception;
  ...
  [
    ...
    pdf.setFillColor(bl.r,bl.g,bl.b);
    pdf.rect(llx, by, 5, 3, 'F');
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(5.5);
    pdf.setTextColor(90,90,64);
