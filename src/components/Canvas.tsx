    ...
    <>
      <KonvaImage
        ...
        onTransformEnd={(e) => {
          ...
          onTransformEnd({ x: node.x(), y: node.y(), scaleX: node.scaleX(), scaleY: node.scaleY() });
        }}
...
// Types that need a capacity prompt on placement
const needsCapacity = ['meeting_room', 'huddle_room', 'archive', 'desk_bench', 'desk_individual'];

export function Canvas() {
  const {
    backgroundImage, backgroundPos, backgroundScale, isEditingBackground,
    setBackgroundPos, setBackgroundScale, versions, activeVersionId,
  ...
  useEffect(() => {
    if (placementMode) setSelectedId(null);
  }, [placementMode]);
  ...
  const handleStageClick = (e: any) => {
    if (placementMode) {
      ...
      const clickedOnBg = e.target.name() === 'background-rect' || e.target.name() === 'background-image';
      if (clickedOnEmpty || clickedOnBg) setSelectedId(null);
    }
  };
  ...
  return (
    ...
    >
      ...
      {placementMode && (
        <div
          className="absolute top-6 right-6 bg-[#2D2A26] text-white rounded-lg px-4 py-3 flex items-center gap-3 shadow-xl z-20 cursor-pointer pointer-events-auto hover:bg-[#1a1816] transition-colors"
          onClick={() => setPlacementMode(null)}
        >
          <span className="text-xs font-semibold">{dict.clickToPlace}</span>
          <span className="text-[10px] opacity-75 uppercase tracking-widest border-l border-white/20 pl-3">
            {dict.cancelPlacement}
          </span>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-black/20 flex items-center justify-center z-50 pointer-events-auto backdrop-blur-[2px]">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-80">
            <h4 className="font-bold text-lg mb-2 text-gray-900">
              {capacityPrompt.type === 'meeting_room' || capacityPrompt.type === 'huddle_room'
                ? 'Capacidad de la Sala'
                : capacityPrompt.type === 'archive'
                ? 'Cantidad de Mobiliario'
                : 'Capacidad de la Estación'}
            </h4>
            <p className="text-sm text-gray-500 mb-4">
              {capacityPrompt.type === 'archive'
                ? 'Ingresa la cantidad de unidades de archivo'
            ...
            <input
              ...
              defaultValue={
                capacityPrompt.type === 'meeting_room' ? 8
                  : capacityPrompt.type === 'huddle_room' ? 5
                  : capacityPrompt.type === 'desk_bench' ? 2
                  : 1
              }
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
            ...
            <div className="flex justify-end gap-2">
              <button
                className="px-4 py-2 text-sm text-gray-600 hover:bg-[#F8F7F4] rounded-lg font-medium"
                onClick={() => {
              ...
              <button
                className="px-4 py-2 bg-[#5A5A40] hover:bg-[#43432e] text-white text-sm rounded-lg font-medium shadow-sm"
                onClick={() => {
      ...
      <div
        className={`relative border border-[#E5E2DD] bg-white shadow-2xl rounded max-w-full max-h-full overflow-hidden ${placementMode ? 'cursor-crosshair' : 'cursor-default'}`}
              ...
              return (
                <Group
                  ...
                  onTransformEnd={() => {
                    ...
                    updateElementSize(el.id, newW, newH);
                ...
                >
                  <Rect
                    ...
                    strokeWidth={el.type === 'meeting_room' || el.type === 'huddle_room' ? 2 : 1}
                    cornerRadius={4}
                  />
                  {label && (
                    <Text
                      text={`${label}${el.capacity ? `\n(${el.capacity})` : ''}`}
                      width={el.width} height={el.height}
