...
import { Monitor, Users, Briefcase, Sofa, Coffee, Bell, Archive, Server, Laptop, Armchair, Airplay, ChevronDown, ChevronRight } from 'lucide-react';
...
export function Toolbar() {
  ...
  const renderButton = ({ type, icon: Icon }: { type: ElementType, icon: any }) => {
    ...
    return (
      <button 
        ...
        className={`w-full group flex items-center p-3 border rounded-lg transition-all ${isActive ? 'border-[#5A5A40] bg-[#F8F7F4] shadow-inner' : 'border-[#E5E2DD] hover:border-[#5A5A40] hover:bg-[#F8F7F4]'}`}
      >
        <div className={`w-10 h-10 rounded flex items-center justify-center mr-3 transition-colors shrink-0 ${isActive ? 'bg-[#5A5A40] text-white' : 'bg-[#E5E2DD] text-[#5A5A40] group-hover:bg-[#5A5A40] group-hover:text-white'}`}>
          <Icon size={20} />
        </div>
  ...
  return (
    <>
      <h3 className="text-xs font-bold uppercase tracking-widest text-[#A89F91] mb-4">{dict.furnitureLibrary}</h3>
      ...
      <div className="space-y-3">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <div className="text-sm font-semibold text-[#2D2A26]">{language === 'en' ? 'Desks & Workstations' : 'Escritorios'}</div>
                <div className="text-[10px] text-[#A89F91]">4 {language === 'en' ? 'options' : 'opciones'}</div>
              </div>
            </div>
        ...
        {/* Collaborative Seating Accordion */}
        <div className="border border-[#E5E2DD] rounded-lg overflow-hidden">
          <button 
            className="w-full flex items-center justify-between p-3 bg-white hover:bg-[#F8F7F4] transition-colors"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <div className="text-sm font-semibold text-[#2D2A26]">{language === 'en' ? 'Spaces' : 'Espacios'}</div>
                <div className="text-[10px] text-[#A89F91]">4 {language === 'en' ? 'options' : 'opciones'}</div>
              </div>
            </div>
        ...
        {/* Support Buttons Grid / List */}
        <div className="pt-2 space-y-3">
          <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#A89F91] pl-1">{language === 'en' ? 'Support & Enclosed' : 'Soporte y Privados'}</h4>
