    ...
    <div className="flex-1 bg-[#F1EFEC] p-6 overflow-y-auto w-full">
      <div className="max-w-5xl mx-auto space-y-6">
            ...
            <table className="w-full text-sm text-left whitespace-nowrap">
              <thead>
                <tr className="border-b border-[#E5E2DD]">
                  ...
                  {tableData.map((d, i) => (
                    <th key={d.name} className="py-3 px-6 font-semibold text-center"
              ...
              <tbody>
                {[
                  ...
                  const metricKeys = [
                    'seats', 'openSpace', 'offices', 'confRooms',
                    'density', 'daylight', 'privacy', 'efficiency'
                  ] as const;
                  const key = metricKeys[rowIndex];
                  return (
                    <tr key={key}
                        className="border-b border-[#E5E2DD] last:border-0 hover:bg-[#F8F7F4] transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12 w-full">
          ...
          {/* Radar */}
          <div className="bg-white rounded-xl border border-[#E5E2DD] p-6 flex flex-col shrink-0"
               style={{ minHeight: 360 }}>
            <div className="flex items-start justify-between mb-2">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#5A5A40]">
                {dict.summary.compare}
              </h3>
              <div className="flex flex-wrap gap-3 text-[10px] font-bold text-[#A89F91]">
                {tableData.map((d, i) => (
          ...
          <div className="bg-white rounded-xl border border-[#E5E2DD] p-6 flex flex-col shrink-0"
               style={{ minHeight: 360 }}>
            <h3 className="text-xs font-bold uppercase tracking-widest text-[#5A5A40] mb-6">
              {dict.summary.spaceDist}
            </h3>
            ...
            <div className="flex-1 flex flex-col justify-center gap-6 overflow-y-auto pr-2">
              {tableData.map((d, i) => {
                ...
                const amenitySeats = (d.metrics as any).lounge || 0;
                const workPct = Math.round((workSeats / totalSeats) * 100);
                const sharedPct = Math.round(
                  ((d.metrics.seats - workSeats - amenitySeats) / totalSeats) * 100
                );
                const amenityPct = Math.max(0, 100 - workPct - sharedPct);
                ...
                return (
                  <div key={d.name} className="w-full shrink-0">
                    ...
                    <div className="w-full h-8 flex rounded overflow-hidden bg-[#E5E2DD] border border-[#E5E2DD]">
                      <div style={{ width: `${workPct}%` }}
                           className="bg-[#b3d4ff] transition-all duration-500"
                           title={dict.summary.work} />
                      <div style={{ width: `${sharedPct}%` }}
                           className="bg-[#b5e5cf] transition-all duration-500"
                           title={dict.summary.shared} />
                      <div style={{ width: `${amenityPct}%` }}
                           className="bg-[#ffe4a0] transition-all duration-500"
                           title={dict.summary.amenities} />
                      <div className="flex-1 bg-[#ded9d2]"
                           title={dict.summary.structure} />
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex flex-wrap gap-4 justify-center text-[10px] font-bold tracking-wider text-[#A89F91] mt-4 uppercase pt-4 border-t border-[#E5E2DD]">
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-[#b3d4ff]" /> {dict.summary.work}
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-[#b5e5cf]" /> {dict.summary.shared}
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-[#ffe4a0]" /> {dict.summary.amenities}
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-[#ded9d2]" /> {dict.summary.structure}
              </span>
            </div>
          </div>
